 classdef ProductionModel < model.DSGEModel
    ...
    methods
        ...
        function [nextst,outstr]=calcStateTransition(obj,point,solvec,mode,varargin)
            ...
            WIDWL=params.WIDWL;
            fracG=params.fracG;
			rBridge=params.rBridge;
			refiBridge=params.refiBridge;
			APBridge=params.ABridge;
			ACBridge=params.bridge;
			ombar=params.ombar;
			Ibr=params.Ibr;
            fBridge=params.fBridge;
            ...
            WI=point(4) + WIDWL;
            BG=point(5);
            ZA=exogenv.pts_perm(exst,1);
            sig2B=exogenv.pts_perm(exst,2);
            zeta=exogenv.pts_perm(exst,3);
            xi=exogenv.pts_perm(exst,4);
			AStarget=exogenv.pts_perm(exst,5);
			mu_om=exogenv.pts_perm(exst,6);
			Lscale=exogenv.pts_perm(exst,7);
...
%            cB=exp(solvec(4));
            ...
            if FLnodef
				...
				% Bridge loan (if turned on): (1 - omega)*AB
				% Unconditional bridge loan: ABridge (equal to conditional
				% in size if set to ABridge = (1-mu_om)*AB
				% omega*Y_om - bridge*omega*AB -(1-bridge)*AB + ABbridge - wagebill + pibar*KB = 0
				% ASSUME: omstar <= ombar for cases where ombar < Infty
                ACBridge=ACBridge/(wagebill/Y);
                APBridge=APBridge/(wagebill/Y);
				if ombar < Inf
					[OmAbar,OmKbar,~]=obj.Mpayoff_gamma(ombar,mu_om,sig2B); 
                    ACBridge = ACBridge/(1-OmAbar);
                    APBridge = APBridge/(1-OmAbar);
				else
					OmAbar = 0; 
					OmKbar = 0;
				end
                omstar=((1-APBridge-ACBridge)*wagebill + pibar*KB + AB)/(Y_om - wagebill*ACBridge); % NOTE: either APBridge or ACBridge must be zero
            ...
            % producer budget
            bridgeProgram =  ACBridge*(OmA-OmAbar -(OmK -OmKbar))*wagebill ... % conditional loan (inc. asymmetric)
				+ APBridge*(OmA - OmAbar)*wagebill;  % unconditional (untargeted and targeted)
            ...
            N = Gfun(omplus,OmA,tauPi,0) - OmA*(1-tauPi_int+delta*qB)*AB  + (1-OmA)*n0 ...
				- (1-tauPi_int)*(rBridge+fBridge) * bridgeProgram;
            eq_N = equ/N;
            if eqfct_intern
            ...
            bridgeLosses = ACBridge*(1-OmA-(mu_om-OmK))*wagebill + (1-OmA)*APBridge*wagebill;
            MP= OmA*(1-tauPi_int) + Nminus/AB + (1-zeta)*bridgeLosses/AB;
            YB=OmK*ZA*KB^(1-alpha)*L^alpha;                       
                        
            % government debt and taxes
            tauN=tau(BG/Y);
            DebtB=(MP+delta*OmA*qB)*AB;
            WS=DebtB+BG-WI-BG_foreign*BG+WIDWL; 
%            ZA_factor=ZA/mu_ZA;
            ZA_factor=min(1,ZA/mu_ZA*mu_om);
			...
			PsiI = sigmaI/2 * eI^2;
            BR_expense = Pbar * ((1-eta)*zeta*( (1-OmA)*(1-deltaK)*p*KB + outputDWL*omminus*ZA*KB^(1-alpha)*L^alpha ...
												+ bridgeLosses ) ...
            ...
            % intermediary
            rD=1/q-1;
            bI=(WI - phiIWI + eI + rBridge * bridgeProgram - (1-Ibr)*bridgeLosses ...
				- eIcost_intern - qB*AI_g)/(q+shieldI*tauPi*rD-kappa);            
            % saver
            bS=( (1-tau_eff)*wvec(2)*Lbar_scale(2) + WS + Tvec(2) + BR_expense(2) ...
				- cS - qB*AS_g - PsiS)/(q+tauD*rD);          
           
            % next period government debt
            AG_g = max( 0, (1-refiBridge)*AB_g*fracG + refiBridge * (1+rBridge+fBridge)*bridgeProgram	); 
            spending = public_goods + sum(Tvec) + bailout + qB*AG_g + Ibr*bridgeLosses - min(0,fBridge*bridgeProgram);
            taxes = (tau_eff-tauK*OmA)*wagebill + tauK*YB + OmA*AB*(tauPi_int-tauK_int) + ...
				bI*(shieldI*tauPi*rD-kappa) + bS*tauD*rD - tauK*OmA*(deltaK*p + pibar)*KB ...
				 - tauPi_int*(rBridge+fBridge)*bridgeProgram;
            BG_g = (BG + spending - taxes)/q;         
			
			DWL = eta*zeta*((1-OmA)*(1-deltaK)*p*KB + outputDWL*omminus*ZA*KB^(1-alpha)*L^alpha + bridgeLosses) ...
                    + bankDWL*etaI*(1-F_eps)*zetaI*DebtB + PsiI;
            
            if mode>0
                % simulation, mode contains number of next period's state
                exst=mode;
                exnpt=size(exogenv.pts_all,1);
                ...
                ZAnext=exogenv.pts_all(:,1);
                %exnpt=size(obj.Exogenv.pts_all,1);
                sig2B_zeta_xi_next=exogenv.pts_all(:,2:end-1);
			...
			addvars.AG_g=AG_g;
			addvars.tax=taxes;
			addvars.spend=spending;
			addvars.fiscspend=public_goods + sum(Tvec);
			addvars.bailout=bailout;
			addvars.RC=cB+cS+X+public_goods+Psi*KB+DWL-Y;
                     
            outstr=struct;
            outstr.addvars=addvars;
            outstr.exstvec=[ZA;sig2B;zeta;xi;AStarget;mu_om;Lscale];
            outstr.Jvec=[omstar,fom,mu_om,OmK,BG/Y,F_eps,F_eps_cond];
            outstr.fullendogvars=fullendogvars; 
			outstr.params=params;
        end
        ...
        function [fx,J,V]=calcEquations(obj,exst,nextst,solvec,instr,mode,varargin)
			...
			demandShock = params.demandShock;
            AG_g = envec.AG_g;
			
			Lbar_scale=params.Lbar*Lscale;
			...
			U_norm = U_norm .* (1 - demandShock * cvec' );  % u(c - demandShock/2 * c^2)
...
%             nst=size(nextst,1);
            ...
            BGtrans = (BG_g - PayoffI*AG_g)./ mu_G;
            ...
            if params.useJacobian && mode == 0
                ...
                args = [{ZA,sig2B,zeta,xi,AStarget,mu_om,Lscale,ZA_factor},num2cell(fullendogvars), ...
					{qB,X,q,equ,...
...
%             xi=params.xi;
            ...
            % extract some other state-dependent variables
            sig2B_next=obj.Exogenv.pts_perm(nextst(:,1),2);
            zeta_next=obj.Exogenv.pts_perm(nextst(:,1),3);       
			muom_next=obj.Exogenv.pts_perm(nextst(:,1),6);
			
			%size(obj.Exogenv.pts_perm(nextst(:,1),7))
			%size(params.Lbar)
			Lbar_scale=obj.Exogenv.pts_perm(nextst(:,1),7) * params.Lbar';
            ...
            omminus_next=(muom_next-OmK_next);
...
%                                             +(1-zeta_next*outputDWL).*(1-OmK_next).*Y_om_next - (1-OmA_next).*wagebill_next )./AB_next;
            ...
            MPK_next= (1-alpha).* ZAnext .*(KB_next./L).^(-alpha);
            FOCtmp=SDFP.*(OmA_next.*(p_next.*(1-(1-tauK)*deltaK) - pibar) ...
                            + (1-tauK)*OmK_next.*MPK_next ...
                            - fom_next.*Fscript_next.*pibar ...
                            + fom_next.*omstar_next.*MPK_next.*Fscript_next );
		...
		function obj = priceZNS( obj, stv, TOL, MAXIT, varargin )
			...
			while true
				...
				%for i=1:npt
					...
					% cB
					cB = vvec(1);
					payoff = futurePrice(:,2);
					qCB = cB + prnext * ( SDFBtmp .* payoff );

					% cS
					cS = vvec(2);
					payoff = futurePrice(:,3);
					qCS = cS + prnext * ( SDFStmp .* payoff );
...
%             loggr = @(var) diff(log(var)) + log(G(2:end));
            ...
            L= (params.Lbar(:,1).^params.gammaB .* params.Lbar(:,2).^params.gammaS) .* ...
				simseries(:,indexmap.get('Lscale'));
            MPK=(1-params.alpha).*simseries(:,indexmap.get('mu_om')).*(KB./L).^(-params.alpha);
            MPL= params.alpha.*simseries(:,indexmap.get('mu_om')).*(KB./L).^(1-params.alpha);
            Kret = (p(2:end).*(1-deltaK(2:end)) + MPK(2:end)./simseries(2:end,indexmap.get('mu_om'))) ./ p(1:end-1) ./ G(2:end) - 1;
            Kret = log(1+Kret);
...
%             AB_g=simseries(:,indexmap.get('AB_g'));
            ...
            Psi=params.psi/2.*(XbyK-(exp(params.g)-1+deltaK)).^2;
...
%                 - (1-(1-params.theta)*params.tauK + params.delta*qB).*OmA.*AB + qB.*AB_g;
            ...
            tauPi=params.tauPi;
			shieldI=params.shieldI;
            Lret=(MP(2:end) + tauPi(2:end).*(1-theta(2:end)).*OmA(2:end) +deltaB(2:end).*OmA(2:end).*qB(2:end))./qB(1:end-1)-rD(1:end-1)-1;
            Lret_tax=(MP(2:end) +deltaB(2:end).*OmA(2:end).*qB(2:end))./qB(1:end-1)-rD(1:end-1).*(1-shieldI(1:end-1).*tauPi(1:end-1))-1;
            Lret = log(1+Lret);
            ...
            Eret = log( ( N(2:end) + DivP(2:end) ) ./ N(1:end-1)) + log(G(2:end)) - rD(1:end-1);
            ...
            % bridge loan accounting
			mu_om = simseries(:,indexmap.get('mu_om'));
            sig_om = simseries(:,indexmap.get('sig2B'));
            Y = mu_om.*Y_om;          
            [OmAbar,OmKbar]=obj.Mpayoff_gamma(params.ombar,mu_om,sig_om);
            ACBridge = params.bridge./((1-OmAbar).*wagebill./Y);
            APBridge = params.ABridge./((1-OmAbar).*wagebill./Y);
            bridgeLosses = ACBridge.*(1-OmA-(mu_om-OmK)).*wagebill + (1-OmA).*APBridge.*wagebill;
                                    
            zeta=simseries(:,indexmap.get('Zeta'));
            LGD = 1 - ( (1-deltaK).*Drate.*(1-zeta).*p.*KB + ...
						(1-zeta.*params.outputDWL).*(mu_om-OmK).* ...
						ZA.*KB.^(1-params.alpha).*L.^params.alpha - ...
						Drate.*wagebill + (1-zeta).*bridgeLosses )./AB ...
...
%             Ibklev=-bI./mdebt;
			...
			case1 = params.sigmaI > 0 & params.eqfct_intern;
			case2 = params.sigmaI > 0 & ~params.eqfct_intern;
			case3 = params.sigmaI == 0;
			
			cI = ( case1 | case2 ) .* simseries(:,indexmap.get('cI')) + case3 .* ones(size(simseries,1),1);
			eI = case1 .* ( (1-cI)./sigmaI ) + case2 .* ( (cI-1)./sigmaI ) + case3 .* log(simseries(:,indexmap.get('cI')));
			dIcost_intern = case1 .* sigmaI.*eI.^2/2;
			dIcost_extern = case2 .* sigmaI.*eI.^2/2;
            
			phiIWI= params.fixediv .* params.phiI.*params.WIbar + ~params.fixediv .* params.phiI.*WI;
            ...
            bailout=simseries(:,indexmap.get('bailout'));
            ...
            end_equity = ( AI_g(1:end-1) .* (MP(2:end) + deltaB(2:end).*OmA(2:end).*qB(2:end)) - bI(1:end-1));
            start_equity = ( AI_g(1:end-1) .* qB(1:end-1) - (q(1:end-1)+shieldI(1:end-1).*tauPi(1:end-1).*rD(1:end-1)).*bI(1:end-1));
            ...
            franch = VI./WI_end-1;
            %franch(WI<0)=0;
            accROE = (params.F(2:end).*AB(2:end)).*Lret./WI_end(1:end-1);
            %accROE(WI(2:end)<0)=0;
...
%             welfare=VB.^PB .* VS.^PS .* VI.^PI;
            ...
            DWL=params.eta.*zeta.*((1-deltaK).*Drate.*p.*KB ...
                                    + params.outputDWL.*(simseries(:,indexmap.get('mu_om'))-OmK).*ZA.*KB.^(1-params.alpha).*L.^params.alpha ...
                                    + zeta.*bridgeLosses) ...
...
%             rfdebt=simseries(:,indexmap.get('bS')) - min(BG,0);
            ...
            factor = min(1,ZA./params.mu_ZA) .* simseries(:,indexmap.get('mu_om')); %factor=G / params.mu_G;
...
%             Pitax = tauPi.*(1-theta).*OmA.*AB+bI.*params.kappa;
...
%             tauPi_int = params.tauPi.*(1-params.theta);
% 
%     		bridgeProgram = ACBridge.*(OmA-OmAbar -(OmK -OmKbar)).*wagebill ... % conditional loan (inc. asymmetric)
% 				+ APBridge.*(OmA - OmAbar).*wagebill;
%             tax = grosstax - Ktax_ben - Pitax_ben - ...
% 				(params.rBridge+params.fBridge).*tauPi_int.*bridgeProgram;
% 			
% 			AG_g = (1-params.refiBridge).*AB_g.*params.fracG + ...
% 				params.refiBridge.*(1+params.rBridge+params.fBridge).*bridgeProgram; 
% 			
% 			fiscspend = T + gammaG;
% 			
%             spend = T + gammaG + bailout + qB.*AG_g  ...
%                      + params.Ibr.*bridgeLosses - min(params.fBridge,0).*bridgeProgram;
            
			tax = simseries(:,indexmap.get('tax'));
			spend = simseries(:,indexmap.get('spend'));
			fiscspend = simseries(:,indexmap.get('fiscspend'));
			programspend = spend - fiscspend - bailout;
            ...
            % government
            Gmlbt = q.*BG_g;
			Gdbtsvc = BG(2:end) - Gmlbt(1:end-1);
			Gdbtsvc_byY = Gdbtsvc ./ Y_denom(2:end);
            ...
            byVar_table = table(WI,WBm,X,XDWL,KB,wagebill,assets,mdebt,bkdebt,DWL,...
                cB,cI,cS,bI,bIstart,eI,Imasset,tax,Ltax,programspend,...
                fiscspend,spend,Gdef_prim,BG_g,bailout,C,Y,T,gammaG);
            ...
            simseries=[simseries(:,2:end),... state vars
                                      ...
                                      programspend, bailout, Gdef_prim, expERP, expRIE, expERIE, wacc, profbility, acctprof, franch, ...
            ...
            simseries = [simseries,grVar_table{:,:},d_fracS,Gdbtsvc,Gdbtsvc_byY];
            ...
            varnames_add=[{'WB', 'WBm','WSm',... wealth distribution
                      ...
                      grVar_table.Properties.VariableNames,'d_fracS','Gdbtsvc', 'Gdbtsvc_byY'];
    ...
    methods (Static)
        ...
        function [J,fxout] = constructJacobian(params)
            syms ZA sig2B zeta xi AStarget mu_om Lscale ZA_factor
            syms KB LB WI BG
			...
			% use symbolic parameters for algebraic checks
			use_symbolic_params = false;
			params_numeric=params;
			
			if use_symbolic_params
				for p = fieldnames(params)'
					p=p{:};
					val = params.(p);
					if numel(val)>1
						params.(p) = sym(p,size(val));
					else
						params.(p) = sym(p);
					end
				end
				
				% restore numeric values where needed (e.g. flags)
				params.FLnodef=params_numeric.FLnodef;
				params.bridge=params_numeric.bridge;
				params.refiBridge=params_numeric.refiBridge;
				params.eqfct_intern=params_numeric.eqfct_intern;
				params.fixediv=params_numeric.fixediv;
				params.mktConsI=params_numeric.mktConsI;
				params.outputDWL=params_numeric.outputDWL;
			end
...
%             xi=params.xi;
            ...
            if FLnodef
                ...
                omstar=((1-bridge)*AB + wagebill + pibar*KB)/(Y_om+bridge*AB);
            end
            ...
            AB_g = (p*KB_g + bridge*(1+rBridge)*(1-OmK)*AB - (1-phi0)*N - equ + equcost_intern)/qB;
            Nminus = Gfun(omminus,1-OmA,0,zeta);
            DivP = N*(phi0 - eq_N - equcost_extern) - (1-OmA)*n0;
            nstar = (Gfun(omstar,1,tauPi,0) - (1-tauPi*(1-theta)+delta*qB)*AB )/N;
			...
			PsiS = PsiSA + chiLinear * AS_g;
            PsiP = N*phi1/2*eq_N^2;
            BR_expense_all = (1-eta)*(zeta*(1-OmA)*(1-deltaK)*p*KB + zeta*outputDWL*omminus*ZA*KB^(1-alpha)*L^alpha) ...
                              + bankDWL*(1-etaI)*(1-F_eps)*zetaI*DebtB + PsiS + PsiP + pibar*KB;
            ...
            % next period government debt     
			AG_g=(1-refiBridge)*AB_g*fracG + refiBridge*bridge*(1+rBridge)*(1-OmK)*AB;
            spending = public_goods + sum(Tvec) + bailout + qB*AG_g;
			...
			U_normB=U_normB*(1-demandShock*cB);
			U_normS=U_normS*(1-demandShock*cS);
...
%             (3) replace  "D(fOmA)(bunch of stuff)" with "dfOmA" in the Jacobian
            ...
            order=[ZA, sig2B, zeta, xi, AStarget, mu_om, Lscale, ZA_factor, ...
				KB, LB, WI, BG, ...
...
%             xi=params.xi;
            ...
            BR_loss = zeta*(1-OmA)*(1-deltaK)*p*KB_g/G + zeta*params.outputDWL*(1-OmK)*mu_ZA*(KB_g/G)^(1-alpha)*L^alpha ...
                      + bankDWL*zetaI*(1-F_eps)*DebtB + PsiS + pibar*KB_g + PsiP + sigmaI/2*eI^2;
            DWL = eta*zeta* ( (1-OmA)*(1-deltaK)*p*KB_g/G + params.outputDWL*(1-OmK)*mu_ZA*(KB_g/G)^(1-alpha)*L^alpha ) ...
                      + etaI*bankDWL*zetaI*(1-F_eps)*DebtB;
            BR_expense = Pbar*(BR_loss-DWL);
            ...
            bailout=F_eps_plus-(1-F_eps)*(WI - bankDWL*zetaI*DebtB);
            Gsp=gammaG*Y;
            GO=T-(1/G-q)*BG_g-sum(Tvec)-bailout;
