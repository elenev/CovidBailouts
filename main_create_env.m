...
% Get rid of extra parameters defined only for the pandemic case
if expdef.params.prob_pandemic == 0
	expdef.params.sig2_om = expdef.params.sig2_om(1:end-1);
	expdef.params.mu_om = expdef.params.mu_om(1:end-1);
	expdef.params.Lscale = expdef.params.Lscale(1:end-1);
end
...
Lscale_ratio = stv{1}.statsout.Lscale / expdef.params.Lscale(1);
expdef.params.Lscale= Lscale_ratio * expdef.params.Lscale;
...
mpts_perm=[ X(omcomb(:,1)), ...
			Om(omcomb(:,2)), ...
			expdef.params.zeta(omcomb(:,2))', ...
			expdef.params.xivec(omcomb(:,2))', ...
			expdef.params.AStarget(omcomb(:,2))', ...
			expdef.params.mu_om(ones(exnpt,1))', ...
			expdef.params.Lscale(ones(exnpt,1))'];
...
exnames={'ZA','Om','Zeta','xi','chi0','mu_om','Lscale'};

if expdef.params.prob_pandemic(1) > 0
	if length(expdef.params.prob_pandemic)==1
		expdef.params.prob_pandemic(2) = expdef.params.prob_pandemic(1);
	end
	% Define which parameters change in the pandemic state
	pandemic.Om = expdef.params.sig2_om;
	pandemic.mu_om = params.mu_om;
	pandemic.Lscale = params.Lscale;
	
	% For all others, use ZA=1 and low high omega values
	rowIdx = find( mpts_perm(:,1)==1 & mpts_perm(:,2) == expdef.params.sig2_om(2) );
	
	[oldStateContingentVars,colsIdx,~] = intersect(exnames, ...
		fieldnames(pandemic),'stable');
	extraRow = mpts_perm(rowIdx,:);
	extraRow(colsIdx) = cellfun(@(f)pandemic.(f)(end), oldStateContingentVars);
	
	exnpt = exnpt+1;
	mpts_perm = [mpts_perm; extraRow];
	mpts_all = [mpts_all; [extraRow, exnpt] ];
	
	mtrans_pandemic_return = mtrans(rowIdx,:) * (1 - expdef.params.prob_pandemic(2) );
	mtrans_pandemic = mtrans * (1 - expdef.params.prob_pandemic(1) );
	mtrans_pandemic = [ [mtrans_pandemic; mtrans_pandemic_return], ...
		[expdef.params.prob_pandemic(1) * ones(exnpt-1, 1); expdef.params.prob_pandemic(2)] ];
	
	mtrans = mtrans_pandemic;
end
...
IQ_omlo= iqr(makedist('Normal',expdef.params.mu_om(1),sdl));
IQ_omhi= iqr(makedist('Normal',expdef.params.mu_om(1),sdh));
...
% exogenous states (exogenv.exnpt of them)
	...
	if ~isempty(guessobj.mobj.Zfct)
		zguess = guessobj.mobj.Zfct.evaluateAt(inigrid.Pointmat)';
		z_names = guessobj.mobj.Zns_names;
		set_Zns = true;
	else
		set_Zns = false;
	end
...
if strcmp(approxMode,'spline')
	...
	if isequal(guess_mode,'guess') && set_Zns
		Zfct = grid.SplineFunction(inigrid,zguess,knotvec,fbnds);
	end
else
    % build approximating function
    % create guess for solution variable functions
	...
	if isequal(guess_mode,'guess') && set_Zns
		Zfct = grid.LinearInterpFunction(inigrid,zguess);
	end
end
...
if isequal(guess_mode,'guess') && set_Zns
	mobj=instfun(expdef.params,endogenv,exogenv,Vf,Pf,Tf,Tempf,basegrid,J,Zfct);
else
